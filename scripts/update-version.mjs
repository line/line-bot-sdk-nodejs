...
async function updateVersion(newVersion) {
  ...
  const packageJsonData = JSON.parse(
    await fs.readFile(files.packageJson, "utf8"),
  );
  ...
  const packageLockJsonData = JSON.parse(
    await fs.readFile(files.packageLockJson, "utf8"),
  );
...
async function verifyVersion(expectedVersion) {
  ...
  if (packageJsonData.version !== expectedVersion) {
    throw new Error(
      `package.json version mismatch: expected ${expectedVersion}, found ${packageJsonData.version}`,
    );
  }
  ...
  if (packageLockJsonData.version !== expectedVersion) {
    throw new Error(
      `package-lock.json version mismatch: expected ${expectedVersion}, found ${packageLockJsonData.version}`,
    );
  }
  if (
    packageLockJsonData.packages &&
    packageLockJsonData.packages[""] &&
    packageLockJsonData.packages[""].version !== expectedVersion
  ) {
    throw new Error(
      `package-lock.json root package version mismatch: expected ${expectedVersion}, found ${packageLockJsonData.packages[""].version}`,
    );
  }
  ...
  if (
    !versionTsData.includes(
      `const LINE_BOT_SDK_VERSION = "${expectedVersion}";`,
    )
  ) {
    throw new Error(
      `lib/version.ts version mismatch: expected ${expectedVersion}`,
    );
  }
...
async function verifyGitDiff() {
  try {
    ...
    const { addedLines, deletedLines } = numstatOutput
      .trim()
      .split("\n")
      .reduce(
        (acc, line) => {
    ...
    if (addedLines !== 4 || deletedLines !== 4) {
      throw new Error(
        `Unexpected number of changed lines: expected 4 added and 4 deleted, found ${addedLines} added and ${deletedLines} deleted`,
      );
    }

    console.log(
      "Git diff verification passed: 4 lines added and 4 lines deleted.",
    );
